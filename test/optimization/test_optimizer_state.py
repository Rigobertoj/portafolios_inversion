...
from src.optimization import MeanVarianceOptimizer, PostModernOptimizer
...
def test_minimum_variance_updates_instance_weight_after_successful_optimization():
    ...
    assert np.isclose(metrics.portfolio_variance(), result.variance)
    assert np.isclose(metrics.portfolio_annual_volatility(), result.volatility)
...
def test_minimum_semivariance_updates_instance_weight_after_successful_optimization():
    ...
    assert result.success
    np.testing.assert_allclose(portfolio.weight, result.weights)
    assert np.isclose(result.downside_risk, np.sqrt(result.semivariance))
    assert np.isclose(result.objective_value, result.semivariance)
    assert result.weights_by_ticker.index.tolist() == portfolio.tickers
