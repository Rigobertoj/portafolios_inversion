...
class CorrelationPortfolioSelector:
    ...
    def _as_list(tickers: Iterable[str]) -> list[str]:
        """Normalize a ticker container into an uppercased list."""
    ...
    def _download_prices(self, tickers: Sequence[str]) -> pd.DataFrame:
        """Download and normalize prices for the provided tickers."""
    ...
    def _apply_coverage_filter(self, prices: pd.DataFrame) -> pd.DataFrame:
        """Keep only assets whose price history meets the coverage threshold."""
        if prices.empty:
            return prices
    ...
    def _compute_corr_scores(
        ...
        """Compute a mean peer-correlation score for each ticker."""
        if corr.shape[1] < 2:
            return pd.Series(dtype=float)

        scores: dict[str, float] = {}
        for ticker in corr.columns:
            peers = [column for column in corr.columns if column != ticker]
            ...
            if weights:
                ...
                if np.allclose(peer_weights.sum(), 0.0):
                    scores[ticker] = float(values.mean())
                else:
                    scores[ticker] = float(
    ...
    def rank_within_groups(
        ...
        for group, tickers in grouped_tickers.items():
            ...
            prices = self._apply_coverage_filter(self._download_prices(tickers_clean))
            if prices.shape[1] < 2:
                continue
            ...
            scores = self._compute_corr_scores(returns.corr(), weights=group_weights)
            if scores.empty:
                continue
    ...
    def build_multigroup_portfolio(
        ...
        prices = self._apply_coverage_filter(self._download_prices(candidate_tickers))
        returns = prices.pct_change(fill_method=None).dropna(how="all")
        corr = returns.corr()
        ...
        if len(final_tickers) > 1:
            ...
            values = final_corr.to_numpy()[tri_idx]
            if self.use_absolute_corr:
                values = np.abs(values)
            final_mean_offdiag_corr = float(np.nanmean(values))
    ...
    def update_portfolio(
        ...
        candidates = [ticker for ticker in self._as_list(candidate_tickers) if ticker not in current]
        ...
        prices = self._apply_coverage_filter(self._download_prices(current + candidates))
        returns = prices.pct_change(fill_method=None).dropna(how="all")
        corr = returns.corr()
        ...
        valid_current = [ticker for ticker in current if ticker in corr.columns]
        score_dict: dict[str, float] = {}
        for ticker in candidates:
            if ticker not in corr.columns:
                continue
            peers = valid_current if valid_current else [column for column in corr.columns if column != ticker]
            values = corr.loc[peers, ticker].dropna()
            if not values.empty:
        ...
        candidate_scores = pd.Series(
            score_dict,
            name="corr_with_portfolio",
        ).sort_values()
        new_tickers = candidate_scores.head(max_new_assets).index.tolist()
        updated_tickers = current + [ticker for ticker in new_tickers if ticker not in current]
