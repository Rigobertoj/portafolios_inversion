...
def target_semivar(prices : pd.DataFrame, benchmark : pd.DataFrame = None, umbral : float = 0):
    returns_assets = prices.pct_change().dropna()
    n_assets = len(returns_assets.columns)

    if isinstance(umbral, (int, float)) and float(umbral) != 0:
        diff = returns_assets - float(umbral)
        corr = returns_assets.corr().to_numpy(dtype=float)
        ...
        diff = aligned_assets.sub(aligned_benchmark, axis=0)
        corr = aligned_assets.corr().to_numpy(dtype=float)
    else:
        return None

    below_target = diff.where(diff < 0, 0.0)
    target_downside = below_target.std().to_numpy(dtype=float)
    target_semivar_matrix = np.outer(target_downside, target_downside) * corr

    opt = minimize(
        fun=lambda weight, cov=target_semivar_matrix: float(weight.T @ cov @ weight),
        x0=np.ones(n_assets) / n_assets,
        bounds=[(0, 1)] * n_assets,
        constraints={"fun": lambda weight: np.sum(weight) - 1.0, "type": "eq"},
        tol=1e-50,
    )
    print(opt)
    return opt.x
